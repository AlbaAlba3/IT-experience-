...
class CatBoostDirect(BaseModel):
    ...
    def fit(
        ...
        self.models = []
        print("Проверка для себя - очистил")
        for i in range(self.horizon):
            ...
            train_dataset = cb.Pool(
                data=train_features, label=train_targets[:, i], cat_features=categorical_features_idx
            )
            eval_dataset = cb.Pool(
                data=val_features, label=val_targets[:, i], cat_features=categorical_features_idx
            )
            cb_model.fit(
                train_dataset,
                eval_set=eval_dataset,
            ...
            print(f"Trained model for horizon {i}/{self.horizon}")
    ...
    def predict(self, test_data, id_col="sensor_id", timestamp_col="timestamp", value_col="value"):
        ...
        # Для каждого шага прогноза используем фиксированный размер истории
        # Но берем разные сдвиги в данных
        test_features_idx, target_features_idx = features__test_idx(
                id_column=test_data[id_col],
                series_length=len(test_data),
                model_horizon=self.horizon,
        ...
        col_idx = test_data.columns.get_loc(value_col)
        for i in range(len(self.models)):
            test_predictions = self.models[i].predict(test_features).ravel()
            rows = target_features_idx[:, i].flatten()
            test_data.iloc[rows, col_idx] = test_predictions
